...
declare module 'positron' {
	...
	export type EditorContext = any; // Simplified for standalone use
	 ...
	 */
	/* Commented out for compatibility - AI/Chat features not currently needed
	namespace ai {
		export interface LanguageModelChatProvider<T = any> {
			...
			id: string;
			providerName: string;
			provideLanguageModelChatResponse(model: T, messages: Array<any>, options: any, progress: any, token: vscode.CancellationToken): Thenable<any>;
			provideLanguageModelChatInformation(options: { silent: boolean }, token: vscode.CancellationToken): vscode.ProviderResult<T[]>;
			provideTokenCount(model: T, text: string | any, token: vscode.CancellationToken): Thenable<number>;
